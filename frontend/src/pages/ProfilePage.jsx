...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        console.log("from my profile", response.data.user);
        setUser(response.data.user);
        setAbout(response.data.user.about);
        setConfAbout(response.data.user.about);
        setIsUser(response.data.user._id == localStorage.getItem("userID"));
      } catch (error) {
  ...
  const handleAboutUpdate = async () => {
    try {
      let response = await axios.put(
        "http://localhost:5000/api/user/about",
        { about },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  return (
    <div className="profile-page">
      ...
      <p>(Click image to upload/change it)</p>
            ...
            <Row  xs={1} md={2} lg={3} className="g-4">
              {user.blogs.map((blog) => (
                <Col  key={blog._id}>
                  <Card>
                    <Card.Img
                      style={{backgroundColor:"gray",maxWidth: "500px"}} 
                      variant="top"
                      src={
                        blog.image
